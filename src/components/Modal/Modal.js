...
function Modal({active, setActive, text}) {
  return (
    <div className={`${active ? styles["modal-overlay_active"] : styles["modal-overlay"]} `} onClick={() => setActive(false)}>
      <div className={styles["modal"]} onClick={(e) => e.stopPropagation()}>
        <h1 className={styles["modal__title"]}>
          {text}
        </h1>
        <button className={styles["modal__btn-close"]} onClick={() => setActive(false)}>
